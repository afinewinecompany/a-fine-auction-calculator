...
import { LeagueSettings, Player, DraftedPlayer } from '../lib/types';
...
export function DraftRoom({ settings, players: initialPlayers, onComplete }: DraftRoomProps) {
  ...
  const [myRoster, setMyRoster] = useState<DraftedPlayer[]>([]);
  const [allDrafted, setAllDrafted] = useState<DraftedPlayer[]>([]);
