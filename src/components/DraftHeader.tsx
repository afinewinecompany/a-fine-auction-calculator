          ...
          <div className="flex items-center gap-3 bg-slate-800/50 px-4 py-3 rounded-xl border border-slate-700/50 backdrop-blur-sm">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-blue-400">
                {spotsRemaining} / {totalRosterSpots}
              </span>
            </div>
          </div>
            ...
            {Object.entries(rosterNeedsRemaining).map(
              ([pos, count]) =>
                count > 0 && (
                  <div
                    key={pos}
                    className="px-3 py-1.5 bg-slate-800/70 border border-slate-600 rounded-lg text-slate-300 backdrop-blur-sm"
                  >
          ...
          <div className="flex items-center gap-3 bg-slate-800/50 px-4 py-3 rounded-xl border border-slate-700/50 backdrop-blur-sm">
            <div className="flex flex-col">
              ...
              <span className="text-white">
                {totalDrafted} / {totalPlayersNeeded}
              </span>
            </div>
          ...
          <div className="flex items-center gap-3 bg-slate-800/50 px-4 py-3 rounded-xl border border-slate-700/50 backdrop-blur-sm">
            <div
              className={`p-2 rounded-lg bg-gradient-to-br ${
                inflationIndicator.color === 'text-blue-600'
                  ? 'from-blue-600 to-blue-700'
                  : inflationIndicator.color === 'text-yellow-600'
                    ? 'from-yellow-600 to-yellow-700'
                    : inflationIndicator.color === 'text-orange-600'
            ...
            <div className="flex flex-col">
              ...
              >
                {inflationRate >= 0 ? '+' : ''}
                {(inflationRate * 100).toFixed(1)}% ({inflationIndicator.label})
              </div>
            </div>
          </div>
        </div>
