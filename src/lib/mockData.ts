...
export const generateMockPlayers = (): Player[] => {
  ...
  return [...hitters, ...pitchers].map((p, index) => ({
    ...
    projectedStats: {
      HR: 'HR' in p ? p.HR : undefined,
      RBI: 'RBI' in p ? p.RBI : undefined,
      SB: 'SB' in p ? p.SB : undefined,
      AVG: 'AVG' in p ? p.AVG : undefined,
      W: 'W' in p ? p.W : undefined,
      K: 'K' in p ? p.K : undefined,
      ERA: 'ERA' in p ? p.ERA : undefined,
      WHIP: 'WHIP' in p ? p.WHIP : undefined,
      SV: 'SV' in p ? p.SV : undefined
    },
